...
import useRegenSensitivitiesService from "@sensitivities/hooks/use-regen-sensitivities-service";
...
export default function SensitivitiesRegenForm() {
  ...
  const { mutateAsync: regenerateCategories, status: regenerateStatus } =
    useRegenSensitivitiesService({ article_id: articleID });
  ...
  return (
    <AlertDialog
      open={
        dialogStore.isOpen &&
        dialogStore.id === DASHBOARD_DIALOG_ID.sensitivities.regenerate
      }
      onOpenChange={(open) =>
        setDashboardDialogOpen({
          ...dialogStore,
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription className="font-medium">
            This action will change the article's sensitivities status. A notification
          ...
          <Form {...form}>
            <form
              onSubmit={form.handleSubmit(async () => {
                toast.info("Sensitivities are generating in background", {
                  id: "REGEN_SENSITIVITY_" + articleID,
                  duration: 10 * 1000,
                  description:
