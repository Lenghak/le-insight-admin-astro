...
import deleteSensitivitiesAPI from "@sensitivities/services/delete-sensitivities-api";
...
export default function useDeleteSensitivitiesService() {
  const instance = usePrivateQueryInstance();
  const queryClient = useStore($queryClient);

  return useMutation(
    {
      mutationKey: [...sensitivitiesKeys.detail("delete"), instance],
      mutationFn: async ({ id }: { id: string }) =>
        await deleteSensitivitiesAPI({ id }),
      ...
      onError: (error) => {
        ...
        if (error instanceof AxiosError && error.response?.status === 404) {
          title = "Cannot find the sensitivity";
          description =
            "The input in not acceptable. Please check and try again.";
        }
